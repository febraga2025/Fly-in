...
def build_distance_map(map_graph: MapGraph) -> Dict[str, int]:
    ...
    if not map_graph.end_zone:
        raise ValueError("The map does not have a defined end zone.")
    
    distances: Dict[str, int] = {name: 999999 for name in map_graph.zones.keys()}
    ...
    while unvisited:
        ...
        for connection in map_graph.connections:
            if current_zone in (connection.zone_a, connection.zone_b):
                ...
                if not neighbor.can_enter() or neighbor.name not in unvisited:
                    continue
